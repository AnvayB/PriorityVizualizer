...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // ── PARSE MODE ─────────────────────────────────────────────────────────────
    const transcriptInput = form.get("transcript") as string | null;
    const existingSectionsJson = form.get("existingSections") as string | null;

    let transcript = transcriptInput ?? "";

    // Legacy "both" mode: transcribe first then parse
    if (mode === "both") {
      const audioFile = form.get("audio") as File | null;
      if (!audioFile) {
        return new Response(JSON.stringify({ error: "No audio file provided" }), {
...
7. Today is ${today}.
8. tasks arrays may be empty [] if the user only wants to create the section/subsection structure without specific tasks yet.
